...
def get_draft_route(
    ...
    # If no draft exists for the provided ticket id, allow the caller to pass a draft id
    # (frontend occasionally uses the draft id in this route). Try fetching by draft id
    # before returning a 404 to make the endpoint more forgiving.
    if not draft:
        draft = drafts_repo.get_by_id(ticket_id)
    if not draft:
