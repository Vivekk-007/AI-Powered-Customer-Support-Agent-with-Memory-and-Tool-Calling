...
class _DisabledMemoryStore:
    """Fallback memory store when mem0 initialization fails.

    Methods mirror the minimal API used by SupportCopilot and return
    safe defaults so callers don't raise exceptions.
    """

    def search(self, *_, **__):
        return []

    def list_memories(self, *_, **__):
        return []

    def add_resolution(self, *_, **__):
        return None

    def add_interaction(self, *_, **__):
        return None
...
class SupportCopilot:
    def __init__(self, settings: Settings):
        ...
        try:
            ...
            self.memory = _DisabledMemoryStore()
